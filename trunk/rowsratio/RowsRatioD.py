...
class cTerr(object):
    ...
    def __init__ (self, dataDir, mal, mic, nO, sT, kO, sO):
        ...
        self.malFileXls = dataDir + '\\' + self.OKATO+mal+'.xls'    # Имя файла данных малых предприятий.
        self.micFileXls = dataDir + '\\' + self.OKATO+mic+'.xls'    # Имя файла данных микропредприятий.
        ...
        self.numvesmalXls   = 15           # Индекс веса у малых = номер переменной - 1
        ...
        self.numvesmicXls   = 15          # Индекс веса у микро = номер переменной - 1
    ...
    def evalTotal(self, vM, mFile, listOkved, rulOk, \
        ...
        def getDataXls(self):
            if os.access(mFile, os.F_OK):
                ...
                nFirstRow = 1                               # Первая строка с данными в файле территории
...
#            0    1     2       3        4         5       6        7             8        
        ...
        ToPrintLog('Осталось в выборке 2008 по правилам 2007 года         :'+str(len(data))+' предприятий')
        ToPrintLog('Всего исключено из выборки 2008 по правилам 2007 года :'+str(len(data2008)-len(data))+' предприятий')
...
#            print rw
        ...
        ToPrintLog('Осталось в выборке 2008 по правилам 2009 года              :'+str(len(data9))+' предприятий')
        ToPrintLog('Всего исключено из выборки 2008 по правилам 2009 года      :'+str(len(data2008)-len(data9))+' предприятий')
        ToPrintLog('    Из них с нулевыми весами                               :'+str(len([rw for rw in data2008 if rw[1] <= 0])))
...
class TTotal(cTerr):
    ...
    vMikro        = False
    malpok = [
    [1  , 'Количество предприятий за январь-декабрь, штук ',                                                                                'tab_33pred-2007.xls',     'tab_33pred-corr'         ,2 ,'tabl_33_1401(1)_1351000010001-2008.xls',   ''],
    [5  , 'Cредняя численность работников - всего,чел.',                                                                                    'tab_33(01-09)_list1.xls', 'tab_33(01-09)_list1-corr',4 ,'зрм_tabl_33_1401(1)_1121100010001.xls',    ''],
    [8  , 'Выручка (нетто) от реализации товаров, продукции, работ, услуг (без НДС, акцизов и аналогичных обязательных платежей),тыс.руб.', 'tab_33(12)_list1.xls',    'tab_33(12)_list1-corr'   ,13,'выр_tabl_33_1402(1)_2113100020001.xls',    ''],
    [11 , 'Инвестиции в основной капитал (в части новых и приобретенных по импорту основных средств),тыс.руб.',                             'tab_33(13)_list1.xls',    'tab_33(13)_list1-corr'   ,14,'инв_tabl_33_1402(1)_1141100020001.xls',    ''],
    [14 , 'Оборот организации ,тыс.руб.',                                                                                                   'tab_33(o)_list1.xls',     'tab_33(o)_list1-corr'    ,10,'обор_tabl_33_1402(1)_211330002000902.xls', ''],
    [17 , 'Отгружено товаров собственного производства, выполнено работ и услуг собственными силами (без НДС и акциза),тыс.руб.',           'tab_33(06)_list1.xls',    'tab_33(06)_list1-corr'   ,11,'огр_tabl_33_1402(1)_2131100020021.xls',    ''],
    [20 , 'Продано товаров несобственного производства (без НДС и акцизов),тыс.руб.',                                                       'tab_33(07)_list1.xls',    'tab_33(07)_list1-corr'   ,12,'прод_tabl_33_1402(1)_2135000020005.xls',   ''],
    [23 , 'Средняя численность работников списочного состава (без внешних совместителей),чел.',                                             'tab_33(02-09)_list1.xls', 'tab_33(02-09)_list1-corr',5 ,'спис_tabl_33_1401(1)_1121100010003.xls',   ''],
    [26 , 'Фонд начисленной заработной платы внешним совместителям,тыс.руб.',                                                               'tab_33(03-11)_list1.xls', 'tab_33(03-11)_list1-corr',9 ,'tabl_33_1401(1)_2152320020005.xls',''],
    [29 , 'Фонд начисленной заработной платы всех работников,тыс.руб.',                                                                     'tab_33(01-11)_list1.xls', 'tab_33(01-11)_list1-corr',6 ,'tabl_33_1401(1)_2152320020022.xls',''],
    [32 , 'Фонд начисленной заработной платы работников списочного состава (без внешних совместителей),тыс.руб.',                           'tab_33(02-11)_list1.xls', 'tab_33(02-11)_list1-corr',8 ,'tabl_33_1401(1)_2152320020003.xls','']
    ]
    micpok = [
    [1  , 'Количество предприятий за январь-декабрь, штук ',                                                                                                           'tab_33pred-2007.xls',     'tab_33pred-corr'         ,2 ,'КоличествоМикро2008.xls',''],
    ...
    [8  , 'Выручка (нетто) от продажи товаров, продукции, работ, услуг (без НДС, акцизов и аналогичных обязательных платежей),тыс.руб.',                               'tab_33(12)_list1.xls',    'tab_33(12)_list1-corr'   ,13,'',''],
    [11 , 'Инвестиции в основной капитал (в части новых и приобретённых по импорту основных средств) - всего,тыс.руб.',                                                'tab_33(13)_list1.xls',    'tab_33(13)_list1-corr'   ,14,'',''],
    [14 , 'Оборот организаций,тыс.руб.',                                                                                                                               'tab_33(o)_list1.xls',     'tab_33(o)_list1-corr'    ,10,'',''],
    [17 , 'Отгружено товаров собственного производства, выполнено работ и услуг собственными силами (без НДС, акцизов и аналогичных обязательных платежей),тыс.руб.',  'tab_33(06)_list1.xls',    'tab_33(06)_list1-corr'   ,11,'',''],
    [20 , 'Продано товаров несобственного производства (без НДС, акцизов и аналогичных обязательных платежей),тыс.руб.',                                               'tab_33(07)_list1.xls',    'tab_33(07)_list1-corr'   ,12,'',''],
...
#                     sh.cell_value(rowx=i, colx=3).encode('cp1251').strip(), sh.cell_value(rowx=i, colx=4)
            ...
            if kOKATO <= 99:
                ...
                nfoMal2008 = sokato+mal+'.xls'    # Имя файла данных малых предприятий.
                nfoMic2008 = sokato+mic+'.xls'    # Имя файла данных микропредприятий.
...
ToPrintLog("---::  Старт головного модуля.  ::---")

TT = findFilesTerrAllXls()

t0 = time()
ToPrintLog("Старт (Вычисление итогов): %s" % ctime(t0))
